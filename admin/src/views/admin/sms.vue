  ...
  export default {
    ...
    methods: {
      ...
      list(page) {
        ...
        Loading.show();
         axios.post(process.env.VUE_APP_SERVER + '/business/admin/sms/list', {
          page: page,
          size:  this.$refs.pagination.size,
        }).then((response)=>{
          Loading.hide();
          let resp = response.data;
           this.smss = resp.content.list;
      ...
      save() {
        ...
        // 保存校验
        if (1 != 1
          || !Validator.require( this.sms.mobile, "手机号")
          || !Validator.length( this.sms.mobile, "手机号", 1, 50)
          || !Validator.require( this.sms.code, "验证码")
          || !Validator.require( this.sms.use, "用途")
          || !Validator.require( this.sms.at, "生成时间")
          || !Validator.require( this.sms.status, "用途")
        ) {
          return;
        }

        Loading.show();
        axios.post(process.env.VUE_APP_SERVER + '/business/admin/sms/save',  this.sms).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
      ...
      del(id) {
        ...
        Confirm.show("删除短信验证码后不可恢复，确认删除？", function () {
          Loading.show();
          axios.delete(process.env.VUE_APP_SERVER + '/business/admin/sms/delete/' + id).then((response)=>{
            Loading.hide();
            let resp = response.data;
            if (resp.success) {
               this.list(1);
