  ...
  export default {
    ...
    methods: {
      ...
      save() {
        ...
        // 保存校验
        if (1 != 1
          || !Validator.require( this.file.path, "相对路径")
          || !Validator.length( this.file.path, "相对路径", 1, 100)
          || !Validator.length( this.file.name, "文件名", 1, 100)
          || !Validator.length( this.file.suffix, "后缀", 1, 10)
        ) {
          return;
        }

        Loading.show();
        axios.post(process.env.VUE_APP_SERVER + '/file/admin/file/save',  this.file).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
      ...
      del(id) {
        ...
        Confirm.show("删除文件后不可恢复，确认删除？", function () {
          Loading.show();
          axios.delete(process.env.VUE_APP_SERVER + '/file/admin/file/delete/' + id).then((response)=>{
            Loading.hide();
            let resp = response.data;
            if (resp.success) {
               this.list(1);
