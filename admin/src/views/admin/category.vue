  ...
  export default {
    ...
    methods: {
      ...
      save() {
        ...
        // 保存校验
        if (1 != 1
          || !Validator.require( this.category.parent, "父id")
          || !Validator.require( this.category.name, "名称")
          || !Validator.length( this.category.name, "名称", 1, 50)
        ) {
          return;
        }

        Loading.show();
        axios.post(process.env.VUE_APP_SERVER + '/business/admin/category/save',  this.category).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
      ...
      del(id) {
        ...
        Confirm.show("删除分类后不可恢复，确认删除？", function () {
          Loading.show();
          axios.delete(process.env.VUE_APP_SERVER + '/business/admin/category/delete/' + id).then((response)=>{
            Loading.hide();
            let resp = response.data;
            if (resp.success) {
               this.all();
