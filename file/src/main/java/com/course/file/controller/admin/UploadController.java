...
public class UploadController {
    ...
    @GetMapping("/merge")
     public ResponseDto merge() throws Exception {
        ...
        try {
            // 读取第一个分片
            fileInputStream = new FileInputStream(new File(FILE_PATH + "/course/IID79Ful.blob"));
            while ((len = fileInputStream.read(byt)) != - 1) {
                outputStream.write(byt, 0, len);
            }

            // 读取第二个分片
            fileInputStream = new FileInputStream(new File(FILE_PATH + "/course/1JU5xrjY.blob"));
            while ((len = fileInputStream.read(byt)) != - 1) {
                outputStream.write(byt, 0, len);
            }
        } catch (IOException e) {
