      ...
      save() {
        ...
        <#list fieldList as field>
          <#if field.name!="id" && field.nameHump!="createdAt" && field.nameHump!="updatedAt" && field.nameHump!="sort">
            <#if !field.nullAble>
          || !Validator.require( this.${domain}.${field.nameHump}, "${field.nameCn}")
            </#if>
            <#if (field.length > 0)>
          || !Validator.length( this.${domain}.${field.nameHump}, "${field.nameCn}", 1, ${field.length?c})
            </#if>
          </#if>
        </#list>
        ) {
        ...
        Loading.show();
        axios.post(process.env.VUE_APP_SERVER + '/${module}/admin/${domain}/save',  this.${domain}).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
